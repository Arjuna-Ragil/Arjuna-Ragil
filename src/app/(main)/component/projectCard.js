...
export default function ProjectCard({title, desc, color, selector}) {
    ...
    return (
        <div className={`group/card flex w-full translate-y-5 flex-col duration-200 hover:translate-y-0 ${hidden ? "opacity-0" : "opacity-100"}`} onClick={() => {setSelector(selector), setShow(true), setHidden(true)}}>
